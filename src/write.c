...
/*   Updated: 2025/06/24 11:24:18 by hawayda          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "philo.h"

static void	write_status_debug(t_philo *philo, t_philo_status status, long elapsed)
{
	if ((TAKE_FIRST_FORK == status || TAKE_SECOND_FORK == status) && !sumulation_finished(philo->table))
